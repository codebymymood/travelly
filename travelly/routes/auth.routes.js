...
router.post("/auth/signup", (req, res, next) => {
  ...
  if (email == '' && password == ''){
    res.render('../views/profile/auth.hbs', {error: 'Please enter your email and password!'})
    return;
}

  let passwordRegExp = /^(?=.*[A-Za-z])(?=.*\d)[A-Za-z\d]{6,}$/
  if(!passwordRegExp.test(password)){
      res.render('../views/profile/auth.hbs', {error: "Please enter the minimum of 8 characters and at least one letter!"})
      return;
  } 
  ...
  if(!emailRegExp.test(email)){
      res.render('../views/profile/auth.hbs', {error: 'This is not a valid e-mail'})
      return;
  }
  ...
  .then((success)=>{
      res.redirect("../views/profile/auth.hbs");
  })
  .catch((error)=>{
    if (name == ''){
