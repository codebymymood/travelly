...
router.get('/mytrips/:name/:lat/:long', isLogged, (req, res, next) => { ///mytrips/:name/:lat/:long
  ...
  .then((response) => {
    ...
    res.render('trips/destinations.hbs', {name, layout:'logged-in-layout.hbs', attractArr0:attractArr.slice(0,3), attractArr1:attractArr.slice(3,6), attractArr2:attractArr.slice(6,9), attractArr3:attractArr.slice(9,12), attractArr4:attractArr.slice(12,15), attractArr5:attractArr.slice(15,18), attractArr6:attractArr.slice(18,21)})
  })
  .catch((err) => {
    next(err)
  })
